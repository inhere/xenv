...
type Configuration struct {
	...
	//
	//  0: 不做额外向上匹配，仅精确匹配与同线前缀匹配 eg: 输入 1.23.1 时 1.23.2 不匹配；输入 1.23 仍可命中 1.23.2
	//  1: 向上一位匹配(同 minor 线内取更高版本) eg: 1.23.1 -> 1.23.2, 1.23.3, ...
	//  2: 向上两位匹配(同 major 线内取高于输入的最小版本) eg: 1.24.5 -> 1.25.0, 1.26.10, ...
	//  9: 允许所有高版本(取高于输入的最小版本) eg: 1.23.1 -> 1.24.0, 1.24.5, ...
